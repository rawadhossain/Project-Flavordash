...
{                                   // Define parameters for circular loading bar
    const int centerX = 40;         // X coordinate of the center of the circle
    const int centerY = 12;         // Y coordinate of the center of the circle
    const int radius = 8;           // Radius of the circle
    const char completedChar = '*'; // Character for completed part of the loading bar
    const char remainingChar = '.'; // Character for remaining part of the loading bar
        ...
        for (int y = centerY - radius; y <= centerY + radius; y++)
        {
            for (int x = centerX - radius; x <= centerX + radius; x++)
            {
                ...
                if (distance <= radius)
                {
                    ...
                    {
                        printf("%c", completedChar); // Print completed part of the circle
                    }
                    else
                    {
                        printf("%c", remainingChar); // Print remaining part of the circle
                    }
                }
                else
                {
                    printf(" "); // Print empty space outside the circle
                }
            }
            printf("\n\t\t"); // Move to the next line
        }

        Sleep(100); // Pause for a moment

        // Additional delay for specific percentages to slow down the progress
        if (i == 90 || i == 50 || i == 96 || i == 83)
        {
            Sleep(100);
        }
